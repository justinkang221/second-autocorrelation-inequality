#!/usr/bin/env python3
"""
Dinkelbach v2: higher betas, more iterations, progressive resolution.
Target: push past C=0.96259 (current SOTA).
"""
import numpy as np
import torch
import time
...
def dinkelbach_improve(f_np, n_outer=20, n_inner=5000, beta=50000.0):
...
if __name__ == "__main__":
    print("=" * 70)
    print("Dinkelbach v2 optimizer for ACI 2")
    print("=" * 70)
    sys.stdout.flush()
    t0 = time.time()
    ...
    # Strategy 1: Sweep betas with Dinkelbach, more inner iters
    print("\n--- Strategy 1: Dinkelbach beta sweep ---")
    for beta in [20000, 50000, 100000, 200000]:
    ...
    # Strategy 2: Basin-hopping with Dinkelbach at best beta
    print("\n--- Strategy 2: Basin-hopping with best Dinkelbach ---")
    for round_num in range(30):
        ...
        for sigma in [0.5, 0.1, 0.05, 0.01, 0.005, 0.001]:
            f_perturbed = perturb(f_best, sigma, rng)
            f_improved, C_improved = dinkelbach_improve(
                f_perturbed, n_outer=10, n_inner=5000, beta=50000.0
            )
