#!/usr/bin/env python3
"""
SA Surgery v2 adapted for n=1600k. Cross-pollinates from best_1600k.npy.
Expand radii scaled 4x from 100k baseline.
"""
import numpy as np
import torch
import time
...
def stochastic_surgery_round(f_best, n_dips=50, scale_factor=0.001):
    ...
    all_worst_indices = plateau_indices[np.argsort(-deviation)[:worst_k]]
    if len(all_worst_indices) > n_dips:
        worst_indices = np.random.choice(all_worst_indices, size=n_dips, replace=False)
    else:
        worst_indices = all_worst_indices
    f_new = f_best.copy()
    true_scale_base = scale_factor * np.random.uniform(0.8, 1.2)
    for conv_idx in worst_indices:
        ...
        if len(sorted_idx) > 1 and np.random.rand() < 0.2:
            final_pos = i_arr[sorted_idx[1]]; j = j_arr[sorted_idx[1]]
        else:
            final_pos = i_arr[sorted_idx[0]]; j = j_arr[sorted_idx[0]]
        dev_ratio = deviation[np.searchsorted(plateau_indices, conv_idx)] / max_conv
        scale = 1.0 + true_scale_base * dev_ratio
        f_new[final_pos] *= scale
        if final_pos != j: f_new[j] *= scale
    return np.maximum(f_new, 0.0)


if __name__ == "__main__":
    print("=" * 70)
    print("SA Surgery v2 on n=1600k (cross-pollinates from best_1600k.npy)")
    print("=" * 70)
    sys.stdout.flush()
    t0 = time.time()
    ...
    # Scale expand radii 4x for 1600k
    scale = n // 100000
    ...
    for round_num in range(10000):
        # Cross-pollinate from best_1600k.npy
        try:
            ...
            if len(f_shared) == n:
                ...
                if C_shared > C_best:
                    ...
                    if C_shared > C_curr:
                        f_curr = f_best.copy(); C_curr = C_best
        ...
        f_work = stochastic_surgery_round(f_curr, n_dips=n_dips, scale_factor=sf)

        # 6 dk_cycles with random params
        improved_glob = False
        for cycle in range(6):
            t_cycle = temps[np.random.randint(len(temps))]
            expand = expand_schedule[np.random.randint(len(expand_schedule))]
            f_work, C_work = full_dk_cycle(f_work, expand, t_cycle)
            if C_work > C_best:
        ...
        # SA acceptance
        if C_work > C_curr:
            f_curr = f_work.copy(); C_curr = C_work
            stuck_counter = 0
            if not improved_glob:
                print(f"  -> Accepted local: C_curr={C_curr:.13f}")
        else:
            deltaC = C_curr - C_work
            T = 1.0e-7 * np.exp(-stuck_counter / 5.0)
            prob = np.exp(-deltaC / T) if T > 0 else 0
        ...
        if round_num % 10 == 9:
            ...
            rate = (C_best - C_start) / (elapsed + 1)
            print(f"  Round {round_num}: C = {C_best:.13f} "
                  f"[{elapsed:.0f}s, rate={rate:.2e}/s, {n_improvements} impr]")
            sys.stdout.flush()
